...
string decryptVigenere(vector<int> key1, string alphabet1) {
    ...
    if (detected_words>15) { // choose how much words you want in order to be print
        cout<<endl;
        cout<<"Key = ";
...
int main() {
    ...
    int max_key_size = 3; // Here put vigenere maximum keys
    ...
    #pragma omp parallel
    {
        ...
        // Try vigenere1
        #pragma omp for        
        for (int key_size=0;key_size<1;key_size++){ // for all keys
            for (int alphabet=0;alphabet<alphabets.size();alphabet++) {  // for all alphabets
                for (int j=0; j<keys.at(key_size).size();j++){ //for all keys for each vigenere
                    decryptVigenere(keys.at(key_size).at(j),alphabets.at(alphabet)); 
                }
            }
        }
        ...
        // Try vigenere2
        #pragma omp for
        for (int key_size=1;key_size<2;key_size++){ // for all keys
            for (int alphabet=0;alphabet<alphabets.size();alphabet++) {  // for all alphabets
                for (int j=0; j<keys.at(key_size).size();j++){ //for all keys for each vigenere
                    decryptVigenere(keys.at(key_size).at(j),alphabets.at(alphabet)); 
                }
        ...
        for (int key_size=2;key_size<3;key_size++){ // for all keys
            for (int alphabet=0;alphabet<alphabets.size();alphabet++) {  // for all alphabets
                for (int j=0; j<keys.at(key_size).size();j++){ //for all keys for each vigenere
                    decryptVigenere(keys.at(key_size).at(j),alphabets.at(alphabet)); 
                }
